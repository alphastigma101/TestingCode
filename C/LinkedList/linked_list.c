  ...
  for(; ( node.head.s.ptr) != (void*)0; node.head = *node.head.s.ptr ) {
    ...
    */
    ...
    if(node.next.s.size < node.tail.s.size && node.next.s.size >= nunits) {
      if (node.head.s.size < node.next.s.size < node.tail.s.size) {
        printf("=======================================================\n");
    ...
    if ( node.head.s.size == nunits && node.next.s.size == nunits && node.tail.s.size == nunits ) {
      ...
      if (freep == (void*)0) {
        ...
        */
        ...
        printf("=======================================================\n");
        node.next.s.ptr = morecore(nunits);
        freep = &node.next; // You need to set freep to the parent node so it doesn't crash the function free
      }
    }
  }
  return (void*)0;
