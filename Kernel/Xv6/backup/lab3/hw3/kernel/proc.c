...
void FATCAT_SCHED(struct proc *p, uint64 current_tick) {
  ...
  //printf("%d\t\%s\t", p->last_scheduled_tick, "Last Tick\n");
  //printf("%d\t%s\t", tick_diff * -1, "Tick Difference!");
  if ( p->token_bucket <= 64 && tick_diff > 0) {
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        if(FATCAT_SCHEDULER == 1) { FATCAT_SCHED(p, current_tick); }
        else {
...
int pslist(uint64 utable) {
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    int tick_diff = p->time - p->last_scheduled_tick; // Get the diff again
    ...
    if (tick_diff > 0) {
      ...
      for (int i = 0; i < tick_diff; i++) {
        ...
        if (u.avialable_tokens > 0) {
          u.avialable_tokens--;
        }
        else if (u.avialable_tokens < 1) {
          u.avialable_tokens = p->avialable_tokens;
          u.avialable_tokens--;
        }
      }
    }
  ...
  if (FATCAT_SCHEDULER == 1) {
    int ratio = (u.ticks + u.avialable_tokens - u.pticks);
    printf("============================\n");
    printf("Ratio: %d / %d = %.2f\n", ratio, 64, (float)ratio / 64);
    printf("============================\n");
  }
